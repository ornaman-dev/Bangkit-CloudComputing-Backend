...
def create_random_fav_by(db:Session):
    ...
    user_schema = UserCreate(username = email,email=email,password=password,id_rec=id_rec)
    user = create_new_user(user= user_schema,db = db)
    return user

def user_authentication_headers(client: TestClient, email: str, password: str, id_rec: str):
    """
...
def authentication_token_from_email(client: TestClient, email: str, db: Session):
    ...
    if not user:
        ...
        user_in_create = UserCreate(username=email, email=email, password=password, id_rec=id_rec)
        user = create_new_user(user=user_in_create, db=db)
    else:
        # Jika pengguna ditemukan, perbarui nilai id_rec
        id_rec = user.id_rec
    return user_authentication_headers(client=client, email=email, password=password, id_rec=id_rec)
